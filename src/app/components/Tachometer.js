...
function SetSvgProps(phi, rx, ry, t1, cx, cy, delta) {
    ...
    return ` M ${sX} ${sY} A ${rx} ${ry} ${phi / (2*π) * 360} ${fA} ${fS} ${eX} ${eY}`
}
...
function Tachometer(props) {
    ...
    return (
        <div>
            <svg>
                <path d={SetSvgProps(0, 100, 100, 0, 200, 200, 240)} stroke="yellow" strokeWidth="5" fill="none"/>
                <PieChart data={[1,3,4,2]} outerRadius={90} innerRadius={80}/>
            </svg>
        </div>
    );
}
